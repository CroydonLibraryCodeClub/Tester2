...
def  main():
  ...
  installer = "\installer\Tester Installer.exe"
   
  if demo:
    installer = "\installer\Tester Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Tester manual.pdf" 
  ]
  ...
  ZIPNAME = "Tester-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Tester-v" + FULLVERSIONSTR + "-win-demo.zip"
